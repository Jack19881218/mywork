...
public class HuaTpUpgradeDialog extends AlertDialog implements OnCheckedChangeListener {
	...
	public static final int MSG_FW_AUTO_UPGRADE = 6;
	...
	public HuaTpUpgradeDialog(HuaHardwareInfoActivity activity) {
	...
	public HuaTpUpgradeDialog(Context context, HuaTouchscreenDevice touchscreenDevice, String fwName) {
		super(context);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mAutoRecovery == true) {
			setTitle(resources.getString(R.string.info_fw_auto_recovery));
		} else if (mAutoUpgrade == true) {
			mCheckBoxAutoUpgrade.setVisibility(View.VISIBLE);
			setTitle(resources.getString(R.string.info_fw_auto_upgrade));
			mDevHitCountdown = -1;
		} else{

			setTitle(resources.getString(R.string.info_fw_upgrade));
			mDevHitCountdown = MSG_FW_AUTO_UPGRADE;
		}

		setButton(BUTTON_POSITIVE, resources.getString(R.string.start_upgrade), mHandler.obtainMessage(BUTTON_POSITIVE));
		setButton(BUTTON_NEUTRAL, resources.getString(R.string.auto_upgrade_setting), mHandler.obtainMessage(BUTTON_NEUTRAL));
	...
	public void autoUpgradeSetting() {
		if (mDevHitCountdown > 0) {
			mDevHitCountdown--;
			if (mDevHitCountdown == 0) {
				mTextView.setText("");
				mCheckBoxAutoUpgrade.setVisibility(View.VISIBLE);
				showToast(R.string.show_auto_upgrade_on, Toast.LENGTH_LONG, false);
			} else if (mDevHitCountdown > 0  && mDevHitCountdown < (MSG_FW_AUTO_UPGRADE-1)) {
				int id = R.string.msg_auto_upgrade_setting ;
				String text = getContext().getResources().getString(id, mDevHitCountdown);
				if (text != null) {
					if (mDialog != null) {
						mDialog.setMessage(text);
						if (mDialog.isShowing() == false) {
				...
				Message message = mHandler.obtainMessage(MSG_FW_AUTO_UPGRADE );
				message.arg1 = mDevHitCountdown;
				mHandler.sendMessageDelayed(message, mDevHitCountdown);
			}
		} else if (mDevHitCountdown < 0) {	
			showToast(R.string.show_auto_upgrade_already, Toast.LENGTH_LONG, false);
		}
	}
}
